...
function generateSecurityArtifacts(project: any) {
  ...
  const secretsMgmt = {
    ...
    content: [
      ...
      ``,
      `## Form Validation for Secret Updates`,
      `- Apply strict validation on both client and server (server is the source of truth).`,
      `- Reject on first invalid field; return structured error with field-level details (redacted).`,
      `- Schema validation (examples: JSON Schema/Zod):`,
      `  - Required fields: name, environment, operation (create|update|rotate|delete), idempotency_token, change_reason.`,
      `  - Unknown keys rejected (stripUnknown=false).`,
      `  - Field constraints:`,
      `    - name: ^[a-z0-9\\-\\/]{3,128}$ (lowercase, dash, slash for namespacing like ${nameSlug}/prod/app)`,
      `    - environment: one of ["dev","staging","prod"] (or your org's list)`,
      `    - idempotency_token: UUID v4 (length=36)`,
      `    - change_reason: 8-200 chars`,
      `    - tags: key/value ASCII only; keys 1-64, values 0-256; total tags <= 20`,
      `- Secret content constraints:`,
      `  - For JSON blobs: must parse; total size <= 64KB; max keys <= 128; values <= 8KB.`,
      `  - Deny-lists & format checks:`,
      `    - Disallow obvious placeholders (e.g., "change-me", "password", "123456").`,
      `    - Enforce format where applicable (JWT-like, URL, DSN) with regex and length bounds.`,
      `  - Prevent high-risk materials (e.g., PEM private keys) unless explicitly allowed by policy and additional approvals.`,
      `- Stage transition validation (Updates/Rotation):`,
      `  - Allowed labels: AWSCURRENT, AWSPREVIOUS, AWSPENDING.`,
      `  - Valid transitions only (e.g., promote PENDING -> CURRENT, demote CURRENT -> PREVIOUS).`,
      `  - Disallow label drift (duplicate CURRENT) and ensure atomic promotion/demotion.`,
      `- Idempotency:`,
      `  - idempotency_token must be unique per secret for 24h; repeat with same token must be safely re-applied or no-op.`,
      `- Approval & change control:`,
      `  - Require change_ticket/reference for prod; enforce 2-person approval rule if configured.`,
      `  - Validate that submitter has permission for the environment and operation.`,
      `- Client-side UX (recommended, server still authoritative):`,
      `  - Real-time field validation with clear inline errors and accessible messages.`,
      `  - Disable submit until valid; show exact constraints (pattern/length) near inputs.`,
      `  - For destructive ops (delete/label changes), require typed confirmation (e.g., UPDATE or DELETE).`,
      `- Telemetry on validation failures:`,
      `  - Log redacted reasons + categories; emit counters (secrets.validation.failure.{reason}).`,
      `  - Never log secret values or sensitive fragments.`,
      ``,
      `## Observability`,
      `- Emit metrics:`,
      `  - secrets.access.success (counter)`,
